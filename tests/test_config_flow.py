...
async def test_async_step_user_success(hass):
    ...
    result = await flow.async_step_user(user_input={CONF_USERNAME: "test-user"})
...
async def test_async_step_browser_success(hass):
    ...
    assert flow.data == {
        CONF_DEVICES: [{"deviceId": "123", "deviceType": "generic"}],
...
async def test_async_step_devices_success(hass):
    ...
    with patch(
        ...
        mock_bhc = AsyncMock()
        mock_bhc.async_get_devices.return_value = [
            {"deviceId": "123", "deviceType": "bhc"},
            {"deviceId": "124", "deviceType": "generic"},
            {"deviceId": "125", "deviceType": "k40"},
        ]
        mock_bhc.refresh_token = "mock_refresh"
        mock_bhc.token = "mock_token"
        mock_create.return_value = mock_bhc
        ...
        result = await flow.async_step_devices(
            user_input={"123_bhc": True, "124_generic": True, "125_k40": False}
        )
...
async def test_async_step_reauth(hass):
    ...
    with patch(
        "custom_components.bosch_homecom.async_setup_entry", return_value=True
    ) as mock_setup_entry, patch(
        "custom_components.bosch_homecom.config_flow.async_get_clientsession",
        return_value=AsyncMock(),
    ), patch(
        ...
        "custom_components.bosch_homecom.async_setup_entry", new_callable=AsyncMock
    ) as mock_create_init:
        # Mock the BHC instance returned by HomeComAlt.create
        mock_bhc = AsyncMock()
        mock_bhc.async_get_devices.return_value = [
...
async def test_async_step_browser_create_failure_returns_form(hass, side_effect):
    """Test browser auth failure during HomeComAlt.create returns form."""
    await setup.async_setup_component(hass, "persistent_notification", {})
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
...
@pytest.mark.asyncio
async def test_async_step_browser_create_unknown_error_returns_form(hass):
    """Test browser auth unknown exception during create returns form."""
    await setup.async_setup_component(hass, "persistent_notification", {})
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
